"""
Uderstanding chains in LangChain V.1
...
def demo_parallel_chain():
    ...
    sentiment_promt = ChatPromptTemplate.from_template("What is the sentiment of the following text?: {text}")
    ...
    analysis_chain = RunnableParallel(
        ...
        sentiment = sentiment_promt | model | parser
    )
    ...
    print("Parallel Analysus Results: ")
...
def demo_passthrough_chain():
    """A chain that demnstrates passthrough functionality."""
...
def demo_chain_branching():
    ...
    general_prompt = ChatPromptTemplate.from_template(
        "You ara  ahelpful assistant, Answer: {input}"
    )

    # classifier
    classifier_prompt = ChatPromptTemplate.from_template(
        "Classify this as 'code' or 'general' : {input}\Return only the classification." 
    )
